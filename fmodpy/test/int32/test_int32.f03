! Test Fortran INTEGER wrapping and usage from Python with fmodpy.

SUBROUTINE TEST_STANDARD(SING_IN, SING_OUT, ARRAY_IN, ARRAY_OUT,&
     KNOWN_ARRAY_OUT, KNOWN_MATRIX_OUT, OPT_SING_IN, OPT_SING_OUT)
  ! Test the basic functionaly of the 'INTEGER' type and its
  ...
  INTEGER, INTENT(IN) :: SING_IN
  INTEGER, INTENT(OUT) :: SING_OUT
  INTEGER, DIMENSION(:), INTENT(IN) :: ARRAY_IN
  INTEGER, DIMENSION(:), INTENT(OUT) :: ARRAY_OUT
  INTEGER, DIMENSION(SIZE(ARRAY_OUT)), INTENT(OUT) :: KNOWN_ARRAY_OUT
  INTEGER, DIMENSION(3,SIZE(ARRAY_OUT)), INTENT(OUT) :: KNOWN_MATRIX_OUT
  INTEGER, INTENT(IN), OPTIONAL :: OPT_SING_IN
  INTEGER, INTENT(OUT), OPTIONAL :: OPT_SING_OUT
  ! Local variable.
  INTEGER :: I
...
FUNCTION TEST_EXTENDED(OPT_ARRAY_IN, KNOWN_OPT_ARRAY_OUT,&
  ...
  ! Test the extended functionaly of the 'INTEGER' type and its
  ! interoperability with Python. This includes, optional array
  ! inputs, optional array outputs, and allocatable array outputs.
  IMPLICIT NONE
  INTEGER, INTENT(IN), OPTIONAL, DIMENSION(:) :: OPT_ARRAY_IN
  INTEGER, INTENT(OUT), OPTIONAL :: KNOWN_OPT_ARRAY_OUT(3)
  INTEGER, INTENT(OUT), OPTIONAL, ALLOCATABLE :: OPT_ALLOC_ARRAY_OUT(:)
  INTEGER, DIMENSION(:), ALLOCATABLE :: ALLOC_ARRAY_OUT
  INTEGER, INTENT(IN) :: N
  ! Local variable.
  INTEGER :: I
