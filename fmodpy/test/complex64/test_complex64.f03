...
SUBROUTINE TEST_STANDARD(SING_IN, SING_OUT, ARRAY_IN, ARRAY_OUT,&
  ...
  COMPLEX(KIND=REAL32), INTENT(IN) :: SING_IN
  COMPLEX(KIND=REAL32), INTENT(OUT) :: SING_OUT
  COMPLEX(KIND=REAL32), DIMENSION(:), INTENT(IN) :: ARRAY_IN
  COMPLEX(KIND=REAL32), DIMENSION(:), INTENT(OUT) :: ARRAY_OUT
  COMPLEX(KIND=REAL32), DIMENSION(SIZE(ARRAY_OUT)), INTENT(OUT) :: KNOWN_ARRAY_OUT
  COMPLEX(KIND=REAL32), DIMENSION(3,SIZE(ARRAY_OUT)), INTENT(OUT) :: KNOWN_MATRIX_OUT
  COMPLEX(KIND=REAL32), INTENT(IN), OPTIONAL :: OPT_SING_IN
  COMPLEX(KIND=REAL32), INTENT(OUT), OPTIONAL :: OPT_SING_OUT
  ! Local variable.
  INTEGER :: I
...
FUNCTION TEST_EXTENDED(OPT_ARRAY_IN, KNOWN_OPT_ARRAY_OUT,&
  ...
  USE ISO_FORTRAN_ENV, ONLY: REAL32
  IMPLICIT NONE
  COMPLEX(REAL32), INTENT(IN), OPTIONAL, DIMENSION(:) :: OPT_ARRAY_IN
  COMPLEX(REAL32), INTENT(OUT), OPTIONAL :: KNOWN_OPT_ARRAY_OUT(3)
  COMPLEX(REAL32), INTENT(OUT), OPTIONAL, ALLOCATABLE :: OPT_ALLOC_ARRAY_OUT(:)
  COMPLEX(REAL32), DIMENSION(:), ALLOCATABLE :: ALLOC_ARRAY_OUT
  INTEGER, INTENT(IN) :: N
  ! Local variable.
  INTEGER :: I
