...
cdef extern:
    void c_test_standard(float* sing_in, float* sing_out, int* array_in_dim_1, float* array_in, int* array_out_dim_1, float* array_out, int* known_array_out_dim_1, float* known_array_out, int* known_matrix_out_dim_1, int* known_matrix_out_dim_2, float* known_matrix_out, bint* opt_sing_in_present, float* opt_sing_in, bint* opt_sing_out_present, float* opt_sing_out)

@cython.binding(True)
def test_standard(float sing_in, float[:] array_in, float[:] array_out, known_array_out=None, known_matrix_out=None, opt_sing_in=None, opt_sing_out=None):
...
! optional outputs.'''
    ...
    if (known_array_out is None):
        known_array_out = numpy.zeros(shape=(array_out.size), dtype='float32', order='F')
        ...
        known_array_out_dim_1 = known_array_out.shape[0]
    cdef float[:] known_array_out_local = known_array_out
    
    # Setting up "known_matrix_out"
    cdef int known_matrix_out_dim_1 = 0
    cdef int known_matrix_out_dim_2 = 0
    if (known_matrix_out is None):
        known_matrix_out = numpy.zeros(shape=(3, array_out.size), dtype='float32', order='F')
        ...
        known_matrix_out_dim_1 = known_matrix_out.shape[0]
        known_matrix_out_dim_2 = known_matrix_out.shape[1]
    cdef float[:,:] known_matrix_out_local = known_matrix_out
    ...
    if (opt_sing_in is None):
        ...
        opt_sing_in = 1
    cdef float opt_sing_in_local = opt_sing_in
    ...
    if (opt_sing_out is None):
        ...
        opt_sing_out = 1
    cdef float opt_sing_out_local = opt_sing_out
...
cdef extern:
    void c_test_extended(bint* opt_array_in_present, int* opt_array_in_dim_1, float* opt_array_in, bint* known_opt_array_out_present, int* known_opt_array_out_dim_1, float* known_opt_array_out, bint* opt_alloc_array_out_present, int* opt_alloc_array_out_dim_1, float** opt_alloc_array_out, int* alloc_array_out_dim_1, float** alloc_array_out, int* n)
...
! inputs, optional array outputs, and allocatable array outputs.'''
    ...
    if (opt_array_in is None):
        ...
        opt_array_in = numpy.zeros(shape=(1), dtype='float32', order='F')
        ...
        opt_array_in_dim_1 = opt_array_in.shape[0]
    cdef float[:] opt_array_in_local = opt_array_in
    ...
    if (known_opt_array_out is None):
        ...
        known_opt_array_out = numpy.zeros(shape=(1), dtype='float32', order='F')
    elif (type(known_opt_array_out) == bool) and (known_opt_array_out):
        known_opt_array_out = numpy.zeros(shape=(3), dtype='float32', order='F')
        ...
        known_opt_array_out_dim_1 = known_opt_array_out.shape[0]
    cdef float[:] known_opt_array_out_local = known_opt_array_out
    ...
    cdef float* opt_alloc_array_out_local
    
    # Setting up "alloc_array_out"
    cdef int alloc_array_out_dim_1
    cdef float* alloc_array_out_local
    ...
    if (opt_alloc_array_out_present):
        opt_alloc_array_out = ptr_to_numpy_array(opt_alloc_array_out_size, numpy.NPY_FLOAT32, &opt_alloc_array_out_local[0])
    ...
    alloc_array_out = ptr_to_numpy_array(alloc_array_out_size, numpy.NPY_FLOAT32, alloc_array_out_local)
