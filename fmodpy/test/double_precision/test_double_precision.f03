...
SUBROUTINE TEST_STANDARD(SING_IN, SING_OUT, ARRAY_IN, ARRAY_OUT,&
  ...
  REAL(KIND=C_DOUBLE), INTENT(IN) :: SING_IN
  REAL(KIND=C_DOUBLE), INTENT(OUT) :: SING_OUT
  REAL(KIND=C_DOUBLE), DIMENSION(:), INTENT(IN) :: ARRAY_IN
  REAL(KIND=C_DOUBLE), DIMENSION(:), INTENT(OUT) :: ARRAY_OUT
  REAL(KIND=C_DOUBLE), DIMENSION(SIZE(ARRAY_OUT)), INTENT(OUT) :: KNOWN_ARRAY_OUT
  REAL(KIND=C_DOUBLE), DIMENSION(3,SIZE(ARRAY_OUT)), INTENT(OUT) :: KNOWN_MATRIX_OUT
  REAL(KIND=C_DOUBLE), INTENT(IN), OPTIONAL :: OPT_SING_IN
  REAL(KIND=C_DOUBLE), INTENT(OUT), OPTIONAL :: OPT_SING_OUT
  ! Local variable.
  INTEGER :: I
...
FUNCTION TEST_EXTENDED(OPT_ARRAY_IN, KNOWN_OPT_ARRAY_OUT,&
  ...
  USE, INTRINSIC :: ISO_C_BINDING, ONLY: C_DOUBLE
  IMPLICIT NONE
  REAL(C_DOUBLE), INTENT(IN), OPTIONAL, DIMENSION(:) :: OPT_ARRAY_IN
  REAL(C_DOUBLE), INTENT(OUT), OPTIONAL :: KNOWN_OPT_ARRAY_OUT(3)
  REAL(C_DOUBLE), INTENT(OUT), OPTIONAL, ALLOCATABLE :: OPT_ALLOC_ARRAY_OUT(:)
  REAL(C_DOUBLE), DIMENSION(:), ALLOCATABLE :: ALLOC_ARRAY_OUT
  INTEGER, INTENT(IN) :: N
  ! Local variable.
  INTEGER :: I
